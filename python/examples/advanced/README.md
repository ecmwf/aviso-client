...
- **02_replay_only.py** -- `mode="replay_only"` with `start_from=<sequence>` for one-shot backfill scripts that exit at end-of-stream rather than keep listening forever. Self-contained: publishes a few notifications under a polygon unique to the script, then replays them.
