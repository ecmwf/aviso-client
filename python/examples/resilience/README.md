...
- **02_error_handling.py** -- deliberately publishes an invalid notification to trigger `HttpError`, then prints the structured fields (`status`, `request_id`, `body`). Demonstrates the `pyaviso.AvisoError` hierarchy for fine-grained vs. catch-all dispatch.
