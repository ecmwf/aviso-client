...
These rules govern the mdBook sources under `docs/`. They keep the book easy to read in an editor and easy to review in a diff. They apply to the Markdown under `docs/src/`, not to this rulebook or other repo prose.
- **Wrap prose at 80 columns.** Every page under `docs/src/` MUST hard-wrap prose, list items, and blockquotes to a maximum line width of 80 columns. Break on whitespace at the limit; do not reflow to one sentence per line. Leave unbroken whatever must stay intact: fenced code blocks, table rows, headings, thematic breaks, block-level HTML, and any single token that overflows on its own (a long URL, an inline-code span, or a `[text](link)` whose parts cannot be split). `docs/src/SUMMARY.md` is exempt because it uses mdBook's table-of-contents grammar, not regular Markdown.
- **Keep the book markdownlint-clean.** Every page MUST pass `markdownlint` under the committed `docs/.markdownlint.jsonc` ruleset, with `docs/src/SUMMARY.md` ignored. Fix the underlying Markdown rather than silencing a finding; NEVER disable a rule inline or per file. A rule is relaxed only in that shared config, only when it genuinely fights the book's content or the 80-column wrap, and always with a one-line reason recorded beside it.
- **The book must build and its examples must pass.** `mdbook build docs` and `mdbook test docs` MUST both succeed before a docs change is committed. `mdbook test` compiles the Rust code blocks, so a broken example fails exactly as a broken test would.
- **Wrap and lint before you stage, not after.** Re-wrap the pages you touched and run the linter as the last step before `git add`, so the committed files already satisfy both. A page left unwrapped, or wrapped but not re-linted, is an unfinished change.
