...
- `.timeout(Duration)` (default `None` for echo, log, and command; `DEFAULT_WEBHOOK_TIMEOUT` (30 s) for the HTTP-based triggers webhook, teams, and post): per-attempt timeout for the command trigger and the HTTP-based triggers. On expiry the command dispatcher kills the shell child and the HTTP dispatchers drop the in-flight request; both return `TriggerError::Timeout(t)`. Echo and log silently ignore the field: each builds a buffer (the JSON body with the newline appended to the same `Vec<u8>`) and submits it via one `write_all` call (locked-stdout `write_all` for echo, `tokio::fs::File::write_all` for log). The `write_all` call itself may internally loop on short writes (it is not guaranteed to be one `write(2)` syscall), but the dispatcher does not race the call against cancellation, so a notification's NDJSON line is never truncated mid-write by a cancellation `select!`. Multi-writer interleaving on a shared log file is a separate concern and is not addressed by the trigger machinery; see [`log` trigger docs](../triggers/log.md) for the honest no-`flock`, multi-syscall caveats.
...
For echo and log triggers, each dispatch attempt is the dispatcher's atomic unit at the cancellation boundary: the supervisor serialises the notification into a `Vec<u8>` once (with the newline appended to the same buffer) and submits the buffer via one `write_all` call. The `write_all` itself may internally loop on short writes (it is not promised as one `write(2)` syscall), but the dispatcher does not race the call against cancellation; this is the same cancellation-boundary contract that applies to `StateStore::put`. Between attempts (during retry backoff sleep) and between triggers (in the per-notification loop), the dispatcher honours both parent-drop and the per-stream cancel oneshot.
