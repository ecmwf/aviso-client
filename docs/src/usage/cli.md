...
```bash
aviso \
  --base-url https://aviso-server.ecmwf.int \
  --token "$AVISO_TOKEN" \
...
| `aviso replay [FILES...]` | Replay historical notifications | Empty stdout; triggers handle output |
...
```yaml
base_url: "https://aviso-server.ecmwf.int"
auth:
  bearer_token: "..."          # OR
  basic:
    username: "alice"
