...
The trait exposes a `refresh()` method that the client calls on a `401 Unauthorized` response. The shipped static-credential providers treat refresh as a no-op: a `401` against a static credential means the credential is wrong and refreshing changes nothing.
