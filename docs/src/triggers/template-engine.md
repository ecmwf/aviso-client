...
| `ValueAfterUnsupportedShellSyntax` | A `{{ notification.<path> }}` in a command after a here-document, `$(( ))`, backticks or `case`, directly after a `$`, inside `${ }`, after a redirection operator, or as the command name; `field` names the reason | `cat <<EOF ... EOF; run {{ notification.sequence }}` |
