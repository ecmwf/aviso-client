...
Spawns `/bin/sh -c <rendered>` per notification, with the notification's fields exposed as `AVISO_*` environment variables. Useful for any operator task that fits in a shell command. **Unix only** (`#[cfg(unix)]`); not available on Windows.
...
The `command:` value runs through the [template engine](./template-engine.md). Two namespaces:
...
Both produce the same result. Style B has the advantage that the command string is shorter and the shell handles quoting.
...
- **Safe**: `command: "exec ./my-binary"` - the shell `exec`s the binary, replacing itself in place, so the kill reaches the binary directly.
