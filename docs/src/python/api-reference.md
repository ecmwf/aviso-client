...
- `listen(event_type=None, *, filter=None, from_=None, mode=None, triggers=None, request=None) -> NotificationIterator` (mode defaults to `"watch"` when not specified; the returned iterator is also a context manager via `with`)
