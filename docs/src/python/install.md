...
That builds the Rust extension into the local virtualenv and makes `import aviso` work. The first build pulls in the workspace's Rust dependencies and compiles them; subsequent builds are incremental.
