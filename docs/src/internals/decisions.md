...
**Cancellation**: `AvisoClient::Drop` cancels all child supervisors. The implementation lands with the follow-up resilience commit (a parent `tokio_util::sync::CancellationToken` or an equivalent `tokio::sync::watch`-based primitive owned by the client; the child supervisors derive a per-task cancel from it). The supervisor task is constructed to own only clones of the bits it needs (`reqwest::Client`, `Url`, `Option<Arc<dyn AuthProvider>>`) rather than a cloned `AvisoClient`; that ensures the parent-token cancellation can actually fire because supervisors do not hold the client alive through refcount.
