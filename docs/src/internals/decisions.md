...
Per-trigger tunables: `retries: u32` (default `0`) with the supervisor's standard exponential backoff schedule, `required: bool` (default `true`), `timeout: Option<Duration>` (default `None`; meaningful only for command, silently ignored on echo/log), and `fail_fast: bool` (default `true`; meaningful only for command, silently ignored on echo/log). The command trigger additionally accepts an env-var map via `.env(k, v)` and an override working directory via `.working_dir(p)`. All four setters live on the parent `Trigger` so the builder API stays uniform; setters that have no effect on a particular kind document that explicitly in their rustdoc.
...
The retry classifier (`is_terminal_error` in the dispatcher) makes `TriggerError::Command { .. }` and `TriggerError::Template { .. }` terminal under `fail_fast = true` (the default) because they are deterministic: the same input produces the same failure, so retrying wastes the budget. `Io`, `Encode`, and `Timeout` stay retryable because they are genuinely transient. `fail_fast = false` keeps every failure retryable.
