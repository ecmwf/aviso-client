...
After running `aviso listen` against `aviso.example.org` with a single listener watching `mars` events filtered on `class=od`, processing one notification, then pressing Ctrl+C, the file looks like:
