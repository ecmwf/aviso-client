...
CI runs on ECMWF self-hosted Linux runners, inside the `eccr.ecmwf.int/aviso/cli-ci` container image, with Rust compilation cached through sccache. Branch protection requires one status check, `ci-pass`. It passes only when every gated job (Rust, cargo-deny, mdBook, the Python matrix, the e2e compose-config check, the C++ binding, and the release-tooling lint/tests) succeeds. The full e2e suite (the real-stack `e2e` job) also runs, but is informational for now and not part of `ci-pass`.
