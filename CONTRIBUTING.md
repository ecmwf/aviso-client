...
CI runs on ECMWF self-hosted Linux runners, inside the `eccr.ecmwf.int/aviso/cli-ci` container image, with Rust compilation cached through sccache. Branch protection requires one status check, `ci-pass`. It passes only when every gated job (Rust, cargo-deny, mdBook, the Python matrix, the e2e compose-config check, the workflow lint, the real-stack `e2e` suite, the C++ binding, and the release-tooling lint/tests) succeeds. The workflow lint runs `actionlint` and `zizmor` over `.github/workflows/` and fails on an action that is not pinned to a commit or a checkout that keeps its credentials; to run it locally, `uv tool run --from actionlint-py actionlint` and `uv tool run zizmor --offline .github/workflows`. The `e2e` job runs the Python, Rust, and C++ suites against a real aviso-server + auth-o-tron + NATS stack; because it shares one stack on the self-hosted host, only one `e2e` job runs at a time repo-wide, so it can queue behind another PR's run.
...
```bash
just release-preflight 2.2.2   # local dry-run gate: version consistency, fmt,
                               # clippy, tests, crate packaging, ordered
                               # publish dry-run, wheel + sdist + twine check
just release-version 2.2.2     # bump the workspace and every internal pin
just release-tag 2.2.2         # annotated bare tag + the push command to run
