...
```bash
just release-preflight 2.1.1   # local dry-run gate: version consistency, fmt,
                               # clippy, tests, crate packaging, ordered
                               # publish dry-run, wheel + sdist + twine check
just release-version 2.1.1     # bump the workspace and every internal pin
just release-tag 2.1.1         # annotated bare tag + the push command to run
