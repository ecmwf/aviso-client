//! Shared tokio runtime used by every sync `PyO3` method.
//!
...
//! methods (when they land in a later commit) use `future_into_py` from
//! the same crate, which schedules on the same runtime.
