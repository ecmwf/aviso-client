...
- A local doc-examples harness that runs every runnable Python code block against `aviso.example.org` (the ECMWF dev server).
...
- **The doc-examples harness** (the maintainer's `/tmp/aviso-py-fact-check/run_doc_examples.py` script that is not part of CI) becomes server-agnostic via `AVISO_BASE_URL`; after the stack lands the harness can point at `http://localhost:8000` instead of `aviso.example.org`.
